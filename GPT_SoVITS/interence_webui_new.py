...
ssl_model = CnhubertModel("GPT_SoVITS/pretrained_models/chinese-hubert-base",device)
ssl_model.load_model()

sovits_model = SovitsModel(sovits_path,device)
sovits_model.load_model()

gpt_model = GptModel(gpt_path,device)
gpt_model.load_model()

def change_sovits_weights(*args, **kwargs):
    pass

def change_gpt_weights(*args, **kwargs):
    pass
...
def get_bert_inf(phones, word2ph, norm_text, language):
    ...
    if language == "zh":
        bert = bert_model.get_bert_feature(norm_text, word2ph)#.to(dtype)
    else:
        bert = torch.zeros(
            (1024, len(phones)),
            dtype=torch.float16 if is_half == True else torch.float32,
...
def get_phones_and_bert(text,language):
    if language in {"en","all_zh","all_ja"}:
        ...
        if language == "zh":
            bert = bert_model.get_bert_feature(norm_text, word2ph)
        else:
            bert = torch.zeros(
                (1024, len(phones)),
                dtype=torch.float16 if is_half == True else torch.float32,
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20, top_p=0.6, temperature=0.6, ref_free = False):
    ...
    with torch.no_grad():
        ...
        ssl_content = ssl_model.model.model(wav16k.unsqueeze(0))[
            "last_hidden_state"
        ].transpose(
            1, 2
        )  # .float()
        codes = sovits_model.model.extract_latent(ssl_content)
    ...
    for text in texts:
        ...
        with torch.no_grad():
            ...
            pred_semantic, idx = gpt_model.model.model.infer_panel(
                all_phoneme_ids,
                all_phoneme_len,
        ...
        refer = sovits_model.get_spepc(ref_wav_path)  # .to(device)
        if is_half == True:
    ...
    yield sovits_model.hps.data.sampling_rate, (np.concatenate(audio_opt, 0) * 32768).astype(
        np.int16
    )
